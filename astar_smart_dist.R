astar_smart_dist <- function (roads, car, packages) {
  "
  ...
  if (car$load == 0) {
    ...
    index = index[1]
    index = index
    # Calculate the distance to all the packages
    closest_pack = 100
    toGo = which(packages[, 5] == 0)
    ...
    # drop of of closest package
    close_drop_x = packages[target, 3]
    close_drop_y = packages[target, 4]
    current_fetch_x = packages[index, offset]
    current_fetch_y = packages[index, 1 + offset]
    h_drop_fetch = abs(close_drop_x - current_fetch_x) + abs(close_drop_y - current_fetch_y)
    if (((closest_pack < 3) && (h_drop_fetch < 3)) || (length(toGo) < 2)) {
